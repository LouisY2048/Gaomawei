...
describe("Pool Contract", function () {
  ...
  describe("addLiquidity", function () {
    ...
    it("should add initial liquidity and mint LP tokens", async function () {
      ...
      await expect(tx)
        ...
        .withArgs(amount0, await token0.getAddress(), amount0, await token1.getAddress(), amount0 * 2n);
    });
    ...
    it("should add liquidity proportionally when pool has reserves", async function () {
      ...
      await expect(tx)
        .to.emit(pool, "AddedLiquidity")
        .withArgs(expectedLP, await token0.getAddress(), addAmount0, await token1.getAddress(), addAmount0*2n);
    });
  ...
  describe("swap", function () {
    ...
    it("should swap Alpha for Beta correctly", async function () {
      ...
      // Perform swap
      const tx = await pool.connect(user).swap(await token0.getAddress(), swapAmount, await token1.getAddress());
      ...
      await expect(tx)
        ...
        .withArgs(await token0.getAddress(), swapAmount, await token1.getAddress(), expectedOutput);
    });

    it("should revert for invalid token pairs", async function () {
      await expect(pool.connect(user).swap(await token0.getAddress(), 100, await token0.getAddress()))
        .to.be.revertedWith("Same tokens");
    });

    it("should revert for zero swap amount", async function () {
      await expect(pool.connect(user).swap(await token0.getAddress(), 0, await token1.getAddress()))
  ...
  describe("getAmountOut", function () {
    ...
    it("should calculate correct output for Token0 to Token1", async function () {
      ...
      const amountOut = await pool.getAmountOut(await token0.getAddress(), amountIn, await token1.getAddress());
    ...
    it("should calculate correct output for Token1 to Token0", async function () {
      ...
      await pool.connect(user).swap(await token0.getAddress(), ethers.parseEther("100"), await token1.getAddress());

      const amountIn = ethers.parseEther("50");
      const amountOut = await pool.getAmountOut(await token1.getAddress(), amountIn, await token0.getAddress());
  ...
  describe("removeLiquidity", function () {
    ...
    it("should remove liquidity correctly", async function () {
      const lpAmount = ethers.parseEther("50"); // Remove half of LP tokens
      const beforeBal0 = await token0.balanceOf(user.address);
      const beforeBal1 = await token1.balanceOf(user.address);

      const tx = await pool.connect(user).removeLiquidity(lpAmount);

      // Check LP tokens burned
      const lpBalance = await pool.balanceOf(user.address);
      expect(lpBalance).to.equal(ethers.parseEther("50")); // 100 - 50
      ...
      // Check user received correct tokens
      const afterBal0 = await token0.balanceOf(user.address);
      const afterBal1 = await token1.balanceOf(user.address);
      expect(afterBal0.sub(beforeBal0)).to.equal(ethers.parseEther("50"));
      expect(afterBal1.sub(beforeBal1)).to.equal(ethers.parseEther("100"));

      // Check event emission
      await expect(tx)
        .to.emit(pool, "RemovedLiquidity")
        .withArgs(lpAmount, await token0.getAddress(), ethers.parseEther("50"), await token1.getAddress(), ethers.parseEther("100"));
    });

    it("should revert when removing zero liquidity", async function () {
      await expect(pool.connect(user).removeLiquidity(0))
        .to.be.revertedWith("Amount must be greater than 0");
    });

    it("should revert when removing more LP tokens than owned", async function () {
      await expect(pool.connect(user).removeLiquidity(ethers.parseEther("150")))
        .to.be.revertedWith("Insufficient LP token balance");
    });
  });
}); 